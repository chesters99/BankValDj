...
class ApisTests(UnitTest):
    ...
    def test_validate_api_valid(self):
        ...
        assert 'Valid Account' in response.content.decode()
    ...
    def test_get_all_rules(self):
        self.load_rules('500000')
        c = APIClient()
        response = c.get(reverse('apis:rule-list'), {})
        assert response.status_code == 200
        assert '500000' in response.content.decode()

    def test_get_a_rule(self):
        self.load_rules('500000')
        rule_id = Rule.objects.get().id
        c = APIClient()
        token = Token.objects.create(user=self.user)
        c.credentials(HTTP_AUTHORIZATION='Token ' + token.key)
        response = c.get(reverse('apis:rule-detail', kwargs={'pk': rule_id}), secure=True)
        assert response.status_code == 200
        assert '500000' in response.content.decode()
    ...
    def test_update_a_rule(self):
        ...
        token = Token.objects.create(user=self.user)
        c.credentials(HTTP_AUTHORIZATION='Token ' + token.key)
        response = c.put(reverse('apis:rule-detail', kwargs={'pk': rule_id}),
    ...
    def test_create_a_rule(self):
        ...
        token = Token.objects.create(user=self.user)
        c.credentials(HTTP_AUTHORIZATION='Token ' + token.key)
        response = c.post('/en/apis/rules/', {'start_sort': '999999', 'end_sort': '999999', 'mod_rule': 'MOD10',
                                           'weight0': 1, 'weight1': 1, 'weight2': 1, 'weight3': 1, 'weight4': 1,
                                           'weight5': 1, 'weight6': 1, 'weight7': 1, 'weight8': 1, 'weight9': 1,
                                           'weight10': 1, 'weight11': 1, 'weight12': 1, 'weight13': 1,
                                           'mod_exception': ''}, secure=True)
    ...
    def test_unauthorized_put_and_post_transactions(self):
        ...
        response = c.put('/en/apis/rules/1/', {'start_sort': '999999', 'end_sort': '999999', 'mod_rule': 'MOD10',
                                            'weight0': 1, 'weight1': 1, 'weight2': 1, 'weight3': 1, 'weight4': 1,
                                            'weight5': 1, 'weight6': 1, 'weight7': 1, 'weight8': 1, 'weight9': 1,
                                            'weight10': 1, 'weight11': 1, 'weight12': 1, 'weight13': 1,
                                            'mod_exception': ''}, secure=True)
