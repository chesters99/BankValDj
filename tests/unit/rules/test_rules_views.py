...
class RulesViewTests(UnitTest):
    ...
    def test_search_rules_view(self):
        ...
        response = self.client.get('/en/rules/search/', {'q': '400000'})
    ...
    def test_loadrules_view(self):
        ...
        response = self.client.post('/en/rules/load/', {'filename': 'valacdos.txt'})
        assert response.status_code == 200
    ...
    def test_detail_view(self):
        ...
        response = self.client.get('/en/rules/detail/%s/' % rule.id)
        assert response.status_code == 200
    ...
    def test_update_view(self):
        ...
        response1 = self.client.post('/en/rules/update/%s/' % rule.id, {'mod_rule': 'DBLAL'})
        ...
        response2 = self.client.get('/en/rules/update/%s/' % rule.id)
    ...
    def test_delete_view(self):
        ...
        response1 = self.client.get('/en/rules/delete/%s/' % rule.id)
        assert response1.status_code == 200
        assert 'Are you sure you want to delete' in response1.content.decode(), response1.content.decode()

        response2 = self.client.post('/en/rules/delete/%s/' % rule.id, follow=True)
        assert response2.status_code == 200
